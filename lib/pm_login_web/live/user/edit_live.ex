defmodule PmLoginWeb.User.EditLive do
  ...
  def mount(_params, %{"curr_user_id" => curr_user_id,"user"=>user}, socket) do
    ...
    str_rights = Enum.map(rights, fn (%Right{} = r)  -> {r.title, r.id} end)

    {:ok,
       socket
       |> assign(changeset: changeset, rights: rights, str_rights: str_rights, user: user, curr_user_id: curr_user_id, show_notif: false, notifs: Services.list_my_notifications_with_limit(curr_user_id, 4)),
  ...
  def render(assigns) do
   PmLoginWeb.UserView.render("edit.html", assigns)
  end
